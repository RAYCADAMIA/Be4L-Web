...
import { BrandPostDetailModal } from './BrandPostDetailModal';
...
export const PartnerFeed: React.FC<PartnerFeedProps> = ({ posts, currentUser, onOpenProfile }) => {
    return (
        <div className="space-y-6 pb-20">
            <h2 className="text-white font-black text-lg px-4 flex items-center gap-2 text-[11px] uppercase tracking-[0.3em] text-white/40">
                <span className="text-electric-teal">BRAND</span> UPDATES
            </h2>
            <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 px-4 pb-4 no-scrollbar">
                {posts.map(post => (
                    <PartnerPostCard key={post.id} post={post} currentUser={currentUser} onOpenProfile={onOpenProfile} />
                ))}
            </div>
        </div>
    );
...
export const PartnerPostCard: React.FC<{ post: PartnerPost; currentUser: User; onOpenProfile: (id: string) => void }> = ({ post, currentUser, onOpenProfile }) => {
    ...
    return (
        <>
            <div
                className="min-w-[280px] w-[280px] md:min-w-[320px] md:w-[320px] bg-zinc-900/50 border border-white/5 rounded-3xl overflow-hidden snap-center flex flex-col shrink-0 cursor-pointer hover:border-white/10 transition-colors group/post"
            ...
            >
                ...
                <div className="p-3 flex items-center justify-between bg-black/20 backdrop-blur-sm">
                    <div
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            onOpenProfile(post.operator_id);
                        }}
                    >
                ...
                {/* Media Carousel */}
                <div className="relative aspect-[4/5] bg-black group/media">
                        ...
                        <>
                            {currentMediaIndex > 0 && (
                                <button
                                    ...
                                    className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white opacity-0 group-hover/media:opacity-100 transition-opacity"
                                >
                                    <ChevronLeft size={16} />
                                </button>
                            )}
                            {currentMediaIndex < post.media_urls.length - 1 && (
                                <button
                                    ...
                                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center text-white opacity-0 group-hover/media:opacity-100 transition-opacity"
                                >

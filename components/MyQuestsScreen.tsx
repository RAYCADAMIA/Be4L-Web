...
const MyQuestsScreen: React.FC<MyQuestsScreenProps> = ({ onBack, onOpenQuest }) => {
    ...
    useEffect(() => {
        ...
        supabaseService.quests.getMyQuests(MOCK_USER.id).then(myQuests => {
            setQuests(myQuests);
            setLoading(false);
        });
    }, []);
    ...
    return (
        <div className="absolute inset-0 z-[50] bg-black flex flex-col animate-in slide-in-from-right duration-300 safe-area-bottom">
            ...
            {/* Header */}
            <div className="p-6 pt-12 flex items-center gap-4 bg-black z-10">
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-24">
                {loading ? (
                    ...
                    sortedQuests.map(q => (
                        <div key={q.id} onClick={() => onOpenQuest(q)} className="mb-4 cursor-pointer hover:opacity-90 transition-opacity">
                            <QuestCard quest={q} onJoin={() => { }} />
                        </div>
                    ))
                ) : (
